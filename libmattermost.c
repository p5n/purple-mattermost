...
mm_set_group_chat(MattermostAccount *ma, const gchar *team_id, const gchar *channel_name, const gchar *channel_id)
{
  if(channel_name != NULL) {
    ...
    printf("mm_set_group_chat: channel name == NULL!");
  }
...
printf("FLAGS=%08X\n", msg_flags);
//          if( /*(msg_flags & PURPLE_MESSAGE_SYSTEM == 0) &&*/
//              (strstr(msg_out, "has left the channel") == NULL) &&
//              (strstr(msg_out, "added to the channel") == NULL) &&
//              (strstr(msg_out, "has joined the channel") == NULL) )
//          {
            purple_serv_got_chat_in(ma->pc, g_str_hash(channel_id), use_username,
                                    msg_flags, msg_out, timestamp);
//          }
          g_free(msg_out);
